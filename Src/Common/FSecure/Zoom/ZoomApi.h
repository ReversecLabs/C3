...
namespace FSecure
{
	...
	public:
		...
		/// Constructor for the Zoom Api class.
		...
		Zoom(std::string const& userAgent, std::string const& account_id, std::string const& client_id, std::string const& client_secret,  std::string const& email, std::string const& vanity_domain, std::string const& channelName, std::string const& proxyOverride);
		...
		/// Write a message to the channel this Zoom object is set to.
		...
		/// Creates a channel on Zoom, if the channel exists already, will call ListChannels internally first to get the channelId.
	...
	private:
		...
		/// The Zoom Bot API token that allows the object access to the workspace. Needs to be manually created as described in documentation.
		...
		/// The Zoom Guild or Server the bot has been added to.
		std::string m_email;

		/// Zoom customers can setup a vanity domain that could be used to domain front.
		std::string m_vanityDomain;
		...
		///  ServerToServer Access token
		std::string m_Token;

		/// Send http request, uses preset token for authentication
		ByteVector SendHttpRequest(std::string const& path, std::wstring const& contentType, std::vector<uint8_t> const& data, WinHttp::Method method);
		ByteVector SendHttpRequest(std::string const& path, WinHttp::ContentType contentType, std::vector<uint8_t> const& data, WinHttp::Method method);
