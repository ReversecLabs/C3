	...
	, m_OutboundDirectionName{ arguments.Read<std::string>() }
{
	...
	auto token = web::http::oauth2::experimental::oauth2_token{};
	token.set_refresh_token(utility::conversions::to_utf16string(arguments.Read<std::string>()));

	// Configure OAuth related options
	auto clientKey = utility::conversions::to_utf16string(arguments.Read<std::string>());
	auto clientSecret = utility::conversions::to_utf16string(arguments.Read<std::string>());
	web::http::oauth2::experimental::oauth2_config oauth2Config(std::move(clientKey), std::move(clientSecret), OBF(L"https://login.windows.net/common/oauth2/v2.0/authorize"), OBF(L"https://login.windows.net/common/oauth2/v2.0/token"), OBF(L""));
	oauth2Config.set_token(std::move(token));
...
size_t FSecure::C3::Interfaces::Channels::Outlook365RestTask::OnSendToChannel(ByteView data)
{
	...
	try
	{
		...
		request.set_body(utility::conversions::to_string_t(jsonBody.dump()));
...
FSecure::ByteVector FSecure::C3::Interfaces::Channels::Outlook365RestTask::OnReceiveFromChannel()
{
	...
	try
	{
		...
		web::http::client::http_client client(utility::conversions::to_string_t(URLwithInboundDirection), m_HttpConfig);
			...
			.then([&](pplx::task<std::wstring> taskData)
		{
			...
			try
			{
				taskDataAsJSON = json::parse(utility::conversions::to_utf8string(taskData.get()));
			}
			...
			for (auto& element : taskDataAsJSON.at(OBF("value")))
			{
				...
				Log({ OBF("OnReceive() for Outlook365RestTask received data of length: ") + std::to_string(element.at(OBF("Body")).at(OBF("Content")).get<std::string>().size()), LogMessage::Severity::DebugInformation });
		...
		pplx::task<void> task = client.request({ web::http::methods::GET }).then([this](web::http::http_response response)
			{
				...
				auto taskDataAsJSON = nlohmann::json::parse(utility::conversions::to_utf8string(response.extract_string().get()));
...
void FSecure::C3::Interfaces::Channels::Outlook365RestTask::RemoveTask(std::string const& id)
{
	...
	auto client = web::http::client::http_client{ utility::conversions::to_string_t(URLwithID), m_HttpConfig };
...
void FSecure::C3::Interfaces::Channels::Outlook365RestTask::RefreshAccessToken()
{
	try
	{
		// Configure the HTTP request to the token endpoint
		auto oa2 = m_HttpConfig.oauth2();
		web::http::client::http_client client(oa2->token_endpoint(), m_HttpConfig);
		web::http::http_request request(web::http::methods::POST);
		request.headers().set_content_type(OBF(L"application/x-www-form-urlencoded"));
		std::string requestBody;
		requestBody += OBF("client_id=");
		requestBody += utility::conversions::to_utf8string(oa2->client_key());
		requestBody += OBF("&refresh_token=");
		requestBody += utility::conversions::to_utf8string(oa2->token().refresh_token());
		requestBody += OBF("&grant_type=refresh_token");
		requestBody += OBF("&client_secret=");
		requestBody += utility::conversions::to_utf8string(oa2->client_secret());
		...
		pplx::task<void> task = client.request(request).then([&](web::http::http_response response)
			{
				...
				tokenCopy.set_access_token(utility::conversions::to_string_t(taskDataAsJSON.at(OBF("access_token")).get<std::string>()));
				tokenCopy.set_refresh_token(utility::conversions::to_string_t(taskDataAsJSON.at(OBF("refresh_token")).get<std::string>()));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const char* FSecure::C3::Interfaces::Channels::Outlook365RestTask::GetCapability()
{
	return R"_(
{
	"create":
